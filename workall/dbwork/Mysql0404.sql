...
insert into test01 values (null,'sonaaa',45,177.9,now(),now());
insert into test01 values (null,'sonb',45,189.3,now(),now());
...
select * from test01 order by 2; -- 컬럼번호 첫컬럼이 1

-- 부분조회
select * from test01 limit  0,3;-- 첫데이타가 0번:0번부터 3개 조회
select * from test01 limit  3,2;-- 첫데이타가 0번:3번부터 2개 조회
...
-- check 에 안맞는값을 insert 할 경우 오류메세지 확인
...
SELECT
    sawon.gender 성별,count(*) 인원수,max(score) 최고점수,
...
insert into cart values (null,'김창인',1,now());
