...
public class Ex5_ServerChat extends JFrame implements Runnable{
	JTextArea area;
	Vector<ClientMember> list = new Vector<ClientMember>();
	...
	public void run() {
		...
		try {
			...
			area.append("Success to make Server Socket\n");
			while (true){
				// 접속한 클라이언트를 허용하고 소켓을 생성
				Socket socket = serverSocket.accept();
				area.append("Connect Ip Address" + socket.getInetAddress().getHostAddress()+"\n");
				this.autoScroll();
				//클라이언트 내부 클래스 생성
			...
			area.append("Fail to make Server Socket\n");
		}
	}
	class ClientMember extends Thread{
		...
		public ClientMember(Socket socket) {
			// 외부의 run 메소드에서 생성시 소켓을 받아서 생성할 것
			mySocket = socket;
		...
		// 각 클라이언트들로부터 들어오는 메세지를 읽어서 모든 클라이언트들한테 보내주는 기능
		@Override
		public void run() {
			super.run();
	...
	public static void main(String[] args) {
		Ex5_ServerChat a = new Ex5_ServerChat("Chatting Server");
		Thread th = new Thread((a)); //String.valueOf 를 사용해야 하는 이유 ?
		th.start();
	}
}
